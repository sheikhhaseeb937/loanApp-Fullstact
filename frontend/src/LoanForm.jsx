...
try {
  const response = await axios.post(`${import.meta.env.VITE_BASE_URL || "http://localhost:8080"}/submit`, formData, {
                ...
                <div className="flex items-end">
                  <div className="w-full bg-gray-100 p-4 rounded-md">
                    <p className="text-sm text-gray-600">
                      Estimated : <span className="font-bold  text-blue-700">
                        {formData.amount && formData.tenure ? 
                          `Rs : ${Math.round((formData.amount / formData.tenure) )}` : 
                          '--'}
                      </span>
                    </p>
